...
| `terrainMaterials.json` | versioned CC0 texture-set provenance, semantic family recipes, and quality residency profiles | Terrain material library (05/export) |
| `terrainMaterialRecipes.json` | versioned per-zone material roles, environmental blend rules, allowed families, and forbidden combinations | Terrain recipes (05/export) |
...
- `controlFields.json` + `control.<continent>.<pack>.rgba`: version 1 exports six four-channel RGBA8 packs (`climate`, `hydrology`, `terrain`, `ecology`, `resources`, `habitat`). The manifest records each channel's field name, continuous range or categorical labels, so clients never hard-code byte interpretation. `habitat.a` is the exact `zone-class` category (`ocean-unclaimed` plus the 16 canonical zones); the viewer keeps this categorical value in a nearest-filtered world-space lookup so coarse LOD triangles cannot interpolate false intermediate zones. The source fields remain float32/integer arrays inside `WorldOutput`; RGBA8 is the browser/asset transport. Continuous fields become streamed terrain attributes and the categorical recipe field remains exact per fragment.
- `terrainMaterials.json`: version 1 exports seven physical CC0 texture sets and 33 semantic terrain families. Every set records provider, upstream asset ID/URL, license, optional physical scan dimensions, and relative albedo/normal/roughness KTX2 paths with color-space metadata. Each family records its shared texture set, tint, physical repeat scale, normal/roughness tuning, blend height, compiler control drivers, and semantic tags. `compatibility`, `balanced`, and `high` residency profiles are part of the contract, so a client loads declared channels rather than assuming the whole disk library is resident. Relative paths are browser content-root paths today; a future desktop cooker/importer resolves the same path under its packaged terrain content root and may keep KTX2 or transcode it to the platform-native GPU format.
- `terrainMaterialRecipes.json`: version 1 exports one recipe for every `zone-class` entry except ocean. Each recipe declares biome membership; allowed family IDs; primary, secondary, tertiary, shore, steep, wet, and cold roles; compiler-field thresholds; macro presentation limits; and forbidden material pairs. The compiler and viewer both reject missing, duplicate, unknown, out-of-order, or internally forbidden recipes. Every one of the 33 Phase 2 families is used by at least one canonical recipe.
