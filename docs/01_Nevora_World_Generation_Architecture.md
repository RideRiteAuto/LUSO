...
/packages/generator/          engine-independent TS compiler
  ...
  /src/environment/             compiler-authored climate, hydrology, geology, ecology, suitability, and resource fields
...
4. **Hydrology** — run a shoreline-seeded priority flood over every land cell, then route flow across that resolved surface and accumulate drainage. This gives every watershed a valid path to open water instead of leaving raw D8 pits or decorative dead ends. Lakes are retained only where the filled depression passes basin-area and depth rules; each exports its computed water surface, spill elevation, shoreline polygon, depth, and outlet. Rivers carry monotonically descending surface profiles and explicit mouth classifications (`open-coast`, `estuary`, `delta`, `lake-inlet`, or `lake-outlet`). Canonical geography may select from those physically valid features—for example Glassmere and Solmara's distributary delta—but cannot bypass the hydrology contract.
5. **Environmental truth** — derive temperature, precipitation, moisture, wetness, drainage, water distance, shoreline influence, slope, exposure, erosion/scree, soil, geology, weather region, exact zone class, buildability, vegetation eligibility, and resource eligibility. Regional biases come from `data/design/environment-regions.json`, which is authored from Bible v11. These fields are compiler truth; renderer noise may only break up their visual presentation. `data/design/terrain-materials.json` defines the semantic material vocabulary and `data/design/terrain-recipes.json` defines each canonical zone's allowed/selected material roles and blend thresholds.
...
12. **Development suitability** — after roads, settlements, and resources exist, refine the compiler's physical buildability field into a housing-suitability field. It rewards naturally moderate terrain with access to roads, water, resources, and settlements while strongly excluding cliffs, swamps, high mountains, and deep flood risk. This stage scores the existing terrain; it never flattens or reshapes it.
13. **POI / lore placement** — place the bible's named landmarks and Lusaran breadcrumbs (inland lighthouse, impossible well, waymarkers, engineered chamber, ...) at zone-appropriate, out-of-the-way locations.
14. **Naming** — assign names to unnamed generated features (minor settlements, landmarks) using a rule-based Portuguese/Iberian-maritime name generator (see doc 03 §5); canonical zone/major-settlement names from the bible are never overwritten.
15. **Export** — write the versioned output contract described in doc 02, including environmental control packs and the renderer-neutral terrain material library.
...
- **Coasts and bathymetry**: ocean elevation now begins near -1 m at the shoreline and follows a super-linear descent through named navigable-shallow, continental-shelf, continental-slope, Luna Sea abyss, and trench bands. World-scale shaping adds a Bruma deep-water hook and a narrow southern trench only after water is already deep, preserving local shore grammar. Beach, rocky-coast, cliff, and estuary terrain selections come from compiler slope/geology/exposure/wetness fields and are independently inspectable. The viewer still applies a small blur to a copy of the heightfield used only for rendered mesh vertices; authoritative height queries remain unsmoothed.
...
**Corrected 2026-08-13 (3rd pass — rift geography and the narrowed sea)**: the 1st pass over-corrected. Pushing the gap to 2× a continent's width did stop the world reading as "one landmass with a strait", but it replaced that with two worlds that merely share a map — nothing about the crossing suggested the halves belong to each other. Kevin, comparing against ArcheAge's Nuia and Haranya: *"they kind of complement each other. Like, their shapes, they almost look like they used to be one mass at some point... Kinda like the whole Pangea idea."* Three changes follow from that, and the current numbers below supersede the 2nd-pass bullets above:

- **The Luna Sea is now a 30000-meter gap** (`lunaSeaGapUnits`), Seradia at `[95536, 0]`. Because both continents' coasts sit inboard of their tile edges, the narrowest open water is ~38.5 km — about 0.7× a continent's land width, which is the proportion ArcheAge uses. The sea is a voyage, not a hemisphere.
- **The facing coasts are cut by a shared rift seam** (`elevation/silhouettes.ts`). A single world-seeded curve — not a per-continent one — decides where each margin falls, so where Valora throws a headland Seradia opens a bay of answering shape. Each margin then adds its own smaller divergence, because a real split shatters and overlaps rather than parting cleanly: the halves rhyme, they never quite mate.
- **The Bruma's radius drops 14000 → 9000 meters.** It was sized against the old 65.5 km sea; at 30 km a 28 km-wide circle spanned the entire crossing, leaving no ordinary water. It is also no longer treated as a keep-out — ships cross it and islands stand in it (Kevin: *"it's almost like the Bermuda Triangle... there's no reason why we couldn't have land there and build stuff in it"*).
