...
No numeric scale exists anywhere in the source material, so this is a default, revisit after the first visual pass. **Corrected 2026-08-10**: the first implementation pass got the *proportions* wrong — it never actually specified how much ocean separates the two continents, so the viewer defaulted to a gap smaller than either continent, which reads as one landmass with a strait, not "two continents divided by a large ocean" (master prompt, "World Scale Requirements"). `data/design/continents.json` is the fix: it is the single source of truth for continent placement and is the file this document should have specified from the start.

- **Per-continent footprint**: **8192 × 8192 world units** per continent tile — large enough that an 8-band, level-1-to-100 continent reads as a subcontinent, not a single dungeon-sized island.
- **The Luna Sea**: the open-water gap between Valora's east coast and Seradia's west coast is **16384 world units — 2× a continent's own width**, not a strait. This is the number that was missing before; it's what makes "large ocean separating continents" and "players should not need to cross the Luna Sea to level, they should want to cross it to profit" (bible §7, §15) actually true rather than aspirational. `data/design/continents.json`'s `worldOffset` fields encode this directly — the viewer reads continent placement from there (via `manifest.json`), it does not invent its own layout.
- **The Bruma**: per the bible, the mysterious central waters of the Luna Sea — placed at the horizontal midpoint of the sea gap, `data/design/continents.json`'s `bruma` entry, with a working radius of **2500 world units** (roughly 15% of a continent's width — a genuinely large, sail-around-it feature, not a pinpoint). It has no landmass (it's mid-ocean) but is exported as a named `seaRegions` record (doc 02 §11) so it's inspectable, and is the intended anchor point for the magical-intensity field's mid-ocean spike (doc 04 §3) once that field is implemented — not built yet, tracked as a gap, not silently dropped.
