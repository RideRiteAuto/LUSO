...
- **Coastal shelf**: the ocean-side depth falloff (how fast elevation drops from the shoreline toward abyssal depth) was steep enough that in the viewer it read as an offshore cliff rather than a beach — the multiplier controlling that rate was softened from 2.2 to 1.1, roughly doubling the horizontal distance the same drop is spread across. That alone only helped marginally, though: most of the visible jaggedness turned out to be inherent to rendering *any* sloped heightfield mesh at a grazing angle, not primarily how steep the data's own slope is. The rest of that fix is viewer-side only (`packages/viewer-threejs/src/terrain.ts`): a small blur applied to a copy of the heightfield used purely for the rendered mesh's vertex positions (never for gameplay-facing height queries, which keep reading the authoritative unsmoothed data). An earlier attempt at the same problem added a separate flat water plane covering the whole world at a fixed sea-level height; that didn't actually fix the boundary line either, and being nearly coincident in extent with the terrain mesh, it z-fought with it across large areas (flickering near real coastlines, a striped Moire pattern out past them) -- removed once the real fix (the height blur) was in place.
